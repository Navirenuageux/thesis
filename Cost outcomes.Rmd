...
```{r}
#Test if the data is normally distributed: Anderson darling test for samples over 5000
ad.test(as.numeric(PD14$total_cost))
#p value < 0.05: not normally distributed
```

```{r echo=F}
#Wilcoxon test: sex (2 groups)
wilcox.test(total_cost ~ sex, data=PD14)
#borderline significant
```

```{r echo=F}
#Kruskal-Wallis test: Ethnicity (bcs many levels)
kruskal.test(data=PD14, total_cost ~ ethnicity)
pwt6 <- pairwise.wilcox.test(PD14$total_cost, PD14$ethnicity, p.adjust.method = "BH")
round(pwt6$p.value, 3)

#Significant, because of Unknown and Other variables: not very helpful
```

```{r echo=F}
#Wilcoxon test: ethnicityD (2 groups)
wilcox.test(total_cost ~ ethnicityD, data=PD14)
#Not significant
```

```{r echo=F}
#Kruskal-Wallis test: imd_decile
kruskal.test(data=PD14, total_cost ~ imd_decile)
#Significant

pwt8 <- pairwise.wilcox.test(PD14$total_cost, PD14$imd_decile, p.adjust.method = "BH")
round(pwt8$p.value, 2)
#Significant: difficult to read, extremes seem to be significant
```

```{r}
#imd2: Kruskal-Wallis test (3 groups)
kruskal.test(data=PD14, total_cost ~ imd2)
#Significant
pwt9 <- pairwise.wilcox.test(PD14$total_cost, PD14$imd2, p.adjust.method = "BH")
round(pwt9$p.value, 5)
#All significant
```
